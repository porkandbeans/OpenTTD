...
static uint GetRoadVehLength(const RoadVehicle *v)
{
	...
	if (e->GetGRF() != nullptr && e->GetGRF()->grf_version >= 8)
	{
		...
		if (veh_len != CALLBACK_FAILED && veh_len >= VEHICLE_LENGTH)
			ErrorUnknownCallbackResult(e->GetGRFID(), CBID_VEHICLE_LENGTH, veh_len);
	}
	else
	{
		/* Use callback 11 */
	...
	if (veh_len == CALLBACK_FAILED)
		veh_len = e->u.road.shorten_factor;
	if (veh_len != 0)
	{
...
void RoadVehUpdateCache(RoadVehicle *v, bool same_length)
{
	...
	for (RoadVehicle *u = v; u != nullptr; u = u->Next())
	{
		...
		if (same_length && veh_len != u->gcache.cached_veh_length)
...
CommandCost CmdBuildRoadVehicle(TileIndex tile, DoCommandFlag flags, const Engine *e, uint16 data, Vehicle **ret)
{
	...
	if (!HasTileAnyRoadType(tile, rti->powered_roadtypes))
...
static FindDepotData FindClosestRoadDepot(const RoadVehicle *v, int max_distance)
{
	...
	{
	case VPF_NPF:
		return NPFRoadVehicleFindNearestDepot(v, max_distance);
	case VPF_YAPF:
...
bool RoadVehicle::FindClosestDepot(TileIndex *location, DestinationID *destination, bool *reverse)
{
	...
	if (destination != nullptr)
		*destination = GetDepotIndex(rfdd.tile);
...
CommandCost CmdTurnRoadVeh(TileIndex tile, DoCommandFlag flags, uint32 p1, uint32 p2, const std::string &text)
{
	...
	if (IsTileType(v->tile, MP_TUNNELBRIDGE) && DirToDiagDir(v->direction) == GetTunnelBridgeDirection(v->tile))
		return CMD_ERROR;
...
inline int RoadVehicle::GetCurrentMaxSpeed() const
{
	...
	return std::min(max_speed, this->current_order.GetMaxSpeed() * 69); // lol
}
...
static RoadVehicle *RoadVehFindCloseTo(RoadVehicle *v, int x, int y, Direction dir, bool update_blocked_ctr = true)
{
	...
	if (update_blocked_ctr && ++front->blocked_ctr > 1480)
		return nullptr;
...
{
	if (!HasTileAnyRoadType(od->tile, od->v->compatible_roadtypes))
		return true;
	...
	if (!HasBit(trackdirbits, od->trackdir) || (trackbits & ~TRACK_BIT_CROSS) || (red_signals != TRACKDIR_BIT_NONE))
		return true;
...
static void RoadVehCheckOvertake(RoadVehicle *v, RoadVehicle *u)
{
	...
	if (IsTileType(v->tile, MP_STATION) || IsTileType(u->tile, MP_STATION))
		return;
	...
	if (v->state >= RVSB_IN_ROAD_STOP || !IsStraightRoadTrackdir((Trackdir)(v->state & RVSB_TRACKDIR_MASK)))
		return;
	 ...
	 */
	od.tile = v->tile;
	if (CheckRoadBlockedForOvertaking(&od))
		return;
...
{
	if (old_z == v->z_pos || _settings_game.vehicle.roadveh_acceleration_model != AM_ORIGINAL)
		return;
...
{
#define return_track(x)           \
	{                             \
		best_track = (Trackdir)x; \
		goto found_best_track;    \
	}
	...
	{
	case VPF_NPF:
		best_track = NPFRoadVehicleChooseTrack(v, tile, enterdir, path_found);
		break;
	case VPF_YAPF:
...
found_best_track:;
	...
	if (HasBit(red_signals, best_track))
...
static bool RoadVehLeaveDepot(RoadVehicle *v, bool first)
{
	...
	{
		if (u->state != RVSB_IN_DEPOT || u->tile != v->tile)
			return false;
	}
	...
	if (first)
	{
		...
		if (RoadVehFindCloseTo(v, x, y, v->direction, false) != nullptr)
			return true;
...
bool IndividualRoadVehicleController(RoadVehicle *v, const RoadVehicle *prev)
{
	...
	RoadDriveEntry rd = _road_drive_data[GetRoadTramType(v->roadtype)][(
																		   ...
																		   (_settings_game.vehicle.road_side << RVS_DRIVE_SIDE)) ^
																	   v->overtaking][v->frame + 1];
	...
	if (rd.x & RDE_NEXT_TILE)
	{
		...
		{
			if (!v->IsFrontEngine())
				error("Disconnecting road vehicle.");
			v->cur_speed = 0;
			return false;
		}
	...
	again:
		...
		if (IsReversingRoadTrackdir(dir))
		{
			...
			if (RoadTypeIsTram(v->roadtype))
			{
				...
				default:
					NOT_REACHED();
				case TRACKDIR_RVREV_NE:
					needed = ROAD_SW;
					break;
				case TRACKDIR_RVREV_SE:
					needed = ROAD_NW;
					break;
				case TRACKDIR_RVREV_SW:
					needed = ROAD_NE;
					break;
				case TRACKDIR_RVREV_NW:
					needed = ROAD_SE;
					break;
				}
	...
	if (rd.x & RDE_TURNED)
	{
		...
		if (RoadTypeIsTram(v->roadtype) && !IsRoadDepotTile(v->tile) && HasExactlyOneBit(GetAnyRoadBits(v->tile, RTT_TRAM, true)))
		{
			...
			default:
				NOT_REACHED();
			case DIAGDIR_NW:
				dir = TRACKDIR_RVREV_SE;
				break;
			case DIAGDIR_NE:
				dir = TRACKDIR_RVREV_SW;
				break;
			case DIAGDIR_SE:
				dir = TRACKDIR_RVREV_NW;
				break;
			case DIAGDIR_SW:
				dir = TRACKDIR_RVREV_NE;
				break;
			}
		}
		else
...
static bool RoadVehController(RoadVehicle *v)
{
	...
	while (j >= adv_spd)
	{
		...
		if (j >= adv_spd && RoadVehCheckTrainCrash(v))
			break;
	}
	...
	if (v->progress == 0)
		v->progress = blocked ? adv_spd - 1 : j;
...
Money RoadVehicle::GetRunningCost() const
{
	...
	if (e->u.road.running_cost_class == INVALID_PRICE)
		return 0;
...
static void CheckIfRoadVehNeedsService(RoadVehicle *v)
{
	...
	if (Company::Get(v->owner)->settings.vehicle.servint_roadveh == 0 || !v->NeedsAutomaticServicing())
		return;
	if (v->IsChainInDepot())
	{
		VehicleServiceInDepot(v);
...
Trackdir RoadVehicle::GetVehicleTrackdir() const
{
	...
	if (this->state > RVSB_TRACKDIR_MASK)
