...
{
	if (!CargoSpec::Get(cargo)->is_freight)
		return 1;
...
void Train::ConsistChanged(ConsistChangeFlags allowed_changes)
{
	...
	for (Train *u = this; u != nullptr; u = u->Next())
	{
		...
		if (!HasBit(e_u->info.misc_flags, EF_RAIL_TILTS))
			train_can_tilt = false;
		...
		if (!u->IsArticulatedPart())
		{
			...
			if ((rvi_u->railveh_type != RAILVEH_WAGON || _settings_game.vehicle.wagon_speed_limits) && !UsesWagonOverride(u))
			{
				...
				if (speed != 0)
					max_speed = std::min(speed, max_speed);
			}
		}
		...
		if (allowed_changes & CCF_CAPACITY)
		{
			...
			if (u->cargo_cap > new_cap)
				u->cargo.Truncate(new_cap);
		...
		else
		{
			...
			if (new_cap != u->cargo_cap)
				ShowNewGrfVehicleError(u->engine_type, STR_NEWGRF_BROKEN, STR_NEWGRF_BROKEN_CAPACITY, GBUG_VEH_CAPACITY, true);
		}
		...
		else
		{
			...
			if (veh_len != u->gcache.cached_veh_length)
...
// yeah I didn't like that behaviour
...
static CommandCost CmdBuildRailWagon(TileIndex tile, DoCommandFlag flags, const Engine *e, Vehicle **ret)
{
	...
	if (!IsCompatibleRail(rvi->railtype, GetRailType(tile)))
		return CMD_ERROR;
...
CommandCost CmdBuildRailVehicle(TileIndex tile, DoCommandFlag flags, const Engine *e, uint16 data, Vehicle **ret)
{
	...
	if (rvi->railveh_type == RAILVEH_WAGON)
		return CmdBuildRailWagon(tile, flags, e, ret);

	/* Check if depot and new engine uses the same kind of tracks *
	 * We need to see if the engine got power on the tile to avoid electric engines in non-electric depots */
	if (!HasPowerOnRail(rvi->railtype, GetRailType(tile)))
		return CMD_ERROR;
	...
	if (flags & DC_EXEC)
	{
		...
		*ret = v;
		...
		if (e->flags & ENGINE_EXCLUSIVE_PREVIEW)
			SetBit(v->vehicle_flags, VF_BUILT_AS_PROTOTYPE);
		...
		if (!HasBit(data, 0) && !(flags & DC_AUTOREPLACE))
...
static void RemoveFromConsist(Train *part, bool chain = false)
{
	...
	if (part->Previous() != nullptr)
		part->Previous()->SetNext(tail->Next());
...
static CommandCost CheckNewTrain(Train *original_dst, Train *dst, Train *original_src, Train *src)
{
	...
	if ((src != nullptr && src->IsEngine() ? 1 : 0) +
			...
			(original_dst != nullptr && original_dst->IsEngine() ? 1 : 0) <=
		0)
	{
		return CommandCost();
	}
		...
		if (!t->IsArticulatedPart() && !t->IsRearDualheaded())
		{
			...
			if (callback != CALLBACK_FAILED)
			{
				...
				if (head->GetGRF()->grf_version < 8)
				{
					...
					if (callback >= 0x100)
						ErrorUnknownCallbackResult(head->GetGRFID(), CBID_TRAIN_ALLOW_WAGON_ATTACH, callback);
				}
				else
				{
					if (callback < 0x400)
...
static void ArrangeTrains(Train **dst_head, Train *dst, Train **src_head, Train *src, bool move_chain)
{
	...
	if (src == *src_head)
	{
		...
		*src_head = move_chain ? nullptr : (src->IsMultiheaded() ? src->GetNextUnit() : src->GetNextVehicle());
	}
...
CommandCost CmdMoveRailVehicle(TileIndex tile, DoCommandFlag flags, uint32 p1, uint32 p2, const std::string &text)
{
	...
	{
		dst_head = dst->First();
		if (dst_head->tile != src_head->tile)
			return CMD_ERROR;
	...
	if (src->IsRearDualheaded())
		return_cmd_error(STR_ERROR_REAR_ENGINE_FOLLOW_FRONT);

	/* When moving all wagons, we can't have the same src_head and dst_head */
	if (move_chain && src_head == dst_head)
		return CommandCost();

	/* When moving a multiheaded part to be place after itself, bail out. */
	if (!move_chain && dst != nullptr && dst->IsRearDualheaded() && src == dst->other_multiheaded_part)
		return CommandCost();

	/* Check if all vehicles in the source train are stopped inside a depot. */
	if (!src_head->IsStoppedInDepot())
		return_cmd_error(STR_ERROR_TRAINS_CAN_ONLY_BE_ALTERED_INSIDE_A_DEPOT);

	/* Check if all vehicles in the destination train are stopped inside a depot. */
	if (dst_head != nullptr && !dst_head->IsStoppedInDepot())
		return_cmd_error(STR_ERROR_TRAINS_CAN_ONLY_BE_ALTERED_INSIDE_A_DEPOT);
	...
	if (flags & DC_EXEC)
	{
		...
		if (original_src_head_front_engine)
			GroupStatistics::CountVehicle(original_src_head, -1);
		if (original_dst_head_front_engine)
			GroupStatistics::CountVehicle(original_dst_head, -1);
		...
		if (src_head != nullptr && src_head->IsFrontEngine())
			GroupStatistics::CountVehicle(src_head, 1);
		if (dst_head != nullptr && dst_head->IsFrontEngine())
			GroupStatistics::CountVehicle(dst_head, 1);
...
static Vehicle *TrainApproachingCrossingEnum(Vehicle *v, void *data)
{
	...
	if (TrainApproachingCrossingTile(t) != tile)
		return nullptr;
...
static bool TrainApproachingCrossing(TileIndex tile)
{
	...
	if (HasVehicleOnPos(tile_from, &tile, &TrainApproachingCrossingEnum))
		return true;
...
static void AdvanceWagonsBeforeSwap(Train *v)
{
	...
	while (length > 2)
	{
		...
		for (int i = 0; i < differential; i++)
			TrainController(first, last->Next());
...
static void AdvanceWagonsAfterSwap(Train *v)
{
	...
	if (leave != nullptr)
	{
		...
		if (d <= 0)
		{
			...
			for (int i = 0; i >= d; i--)
				TrainController(leave, nullptr); // maybe move it, and maybe let another wagon leave
		}
	}
	else
	{
	...
	while (length > 2)
	{
		...
		/* do not update images now */
		for (int i = 0; i < differential; i++)
			TrainController(first, (nomove ? last->Next() : nullptr));
...
void ReverseTrainDirection(Train *v)
{
	...
	if (IsRailDepotTile(v->tile) || IsTileType(v->tile, MP_TUNNELBRIDGE))
		dir = INVALID_DIAGDIR;
	...
	if (UpdateSignalsOnSegment(v->tile, dir, v->owner) == SIGSEG_PBS || _settings_game.pf.reserve_paths)
	{
		...
		if (IsRailDepotTile(v->tile) && TrackdirToExitdir(v->GetVehicleTrackdir()) == GetRailDepotDirection(v->tile))
			first_tile_okay = false;

		if (IsRailStationTile(v->tile))
			SetRailStationPlatformReservation(v->tile, TrackdirToExitdir(v->GetVehicleTrackdir()), true);
...
CommandCost CmdReverseTrainDirection(TileIndex tile, DoCommandFlag flags, uint32 p1, uint32 p2, const std::string &text)
{
	...
	if (p2 != 0)
	{
		...
		if (!HasBit(EngInfo(v->engine_type)->misc_flags, EF_RAIL_FLIPS))
			return CMD_ERROR;
	...
	else
	{
		...
		if ((v->vehstatus & VS_CRASHED) || v->breakdown_ctr != 0)
			return CMD_ERROR;
...
CommandCost CmdForceTrainProceed(TileIndex tile, DoCommandFlag flags, uint32 p1, uint32 p2, const std::string &text)
{
	...
	if (flags & DC_EXEC)
	{
		...
		t->force_proceed = t->force_proceed == TFP_SIGNAL ? TFP_NONE : HasBit(t->flags, VRF_TRAIN_STUCK) || t->IsChainInDepot() ? TFP_STUCK
																																: TFP_SIGNAL;
...
static FindDepotData FindClosestTrainDepot(Train *v, int max_distance)
{
	...
	if (IsRailDepotTile(v->tile))
		return FindDepotData(v->tile, 0);

	PBSTileInfo origin = FollowTrainReservation(v);
	if (IsRailDepotTile(origin.tile))
		return FindDepotData(origin.tile, 0);

	switch (_settings_game.pf.pathfinder_for_trains)
	{
	case VPF_NPF:
		return NPFTrainFindNearestDepot(v, max_distance);
	case VPF_YAPF:
...
bool Train::FindClosestDepot(TileIndex *location, DestinationID *destination, bool *reverse)
{
	...
	if (destination != nullptr)
		*destination = GetDepotIndex(tfdd.tile);
	if (reverse != nullptr)
		*reverse = tfdd.reverse;
...
static void CheckNextTrainTile(Train *v)
{
	...
	if (_settings_game.pf.path_backoff_interval == 255)
		return;
	...
	case OT_GOTO_WAYPOINT:
		...
		if (IsRailWaypointTile(v->tile) && GetStationIndex(v->tile) == v->current_order.GetDestination())
			ProcessOrders(v);
		break;
	...
	if (IsRailStationTile(v->tile) && v->current_order.ShouldStopAtStation(v, GetStationIndex(v->tile)))
		return;
	...
	if (IsTileType(v->tile, MP_RAILWAY) && HasSignalOnTrackdir(v->tile, td) &&
		...
		GetSignalStateByTrackdir(v->tile, td) == SIGNAL_STATE_RED)
		return;

	CFollowTrackRail ft(v);
	if (!ft.Follow(v->tile, td))
		return;
...
void FreeTrainTrackReservation(const Train *v)
{
	...
	if (IsRailDepotTile(tile) && TrackdirToExitdir(td) != GetRailDepotDirection(tile))
		return;
	if (v->track == TRACK_BIT_DEPOT)
	{
	...
	if (TracksOverlap(GetReservedTrackbits(tile) | TrackToTrackBits(TrackdirToTrack(td))))
		return;
	...
	while (ft.Follow(tile, td))
	{
		...
		if (free_tile || (!(ft.m_is_station && GetStationIndex(ft.m_new_tile) == station_id) && !ft.m_is_tunnel && !ft.m_is_bridge))
...
static Track DoTrainPathfind(const Train *v, TileIndex tile, DiagDirection enterdir, TrackBits tracks, bool &path_found, bool do_track_reservation, PBSTileInfo *dest)
{
	...
	{
	case VPF_NPF:
		return NPFTrainChooseTrack(v, path_found, do_track_reservation, dest);
	case VPF_YAPF:
		return YapfTrainChooseTrack(v, tile, enterdir, tracks, path_found, do_track_reservation, dest);
...
static PBSTileInfo ExtendTrainReservation(const Train *v, TrackBits *new_tracks, DiagDirection *enterdir)
{
	...
	while (ft.Follow(tile, cur_td))
	{
		if (KillFirstBit(ft.m_new_td_bits) == TRACKDIR_BIT_NONE)
		{
			...
			if (HasOnewaySignalBlockingTrackdir(ft.m_new_tile, FindFirstTrackdir(ft.m_new_td_bits)))
				break;
		}
		...
		if (target_seen || KillFirstBit(ft.m_new_td_bits) != TRACKDIR_BIT_NONE)
		{
			...
			if (HasReservedTracks(ft.m_new_tile, TrackdirBitsToTrackBits(TrackdirReachesTrackdirs(ft.m_old_td))))
				break;

			/* If we did skip some tiles, backtrack to the first skipped tile so the pathfinder
			 * actually starts its search at the first unreserved tile. */
			if (ft.m_tiles_skipped != 0)
				ft.m_new_tile -= TileOffsByDiagDir(ft.m_exitdir) * ft.m_tiles_skipped;

			/* Choice found, path valid but not okay. Save info about the choice tile as well. */
			if (new_tracks != nullptr)
				*new_tracks = TrackdirBitsToTrackBits(ft.m_new_td_bits);
			if (enterdir != nullptr)
				*enterdir = ft.m_exitdir;
		...
		if (IsSafeWaitingPosition(v, tile, cur_td, true, _settings_game.pf.forbid_90_deg))
		{
			...
			if (!(wp_free && TryReserveRailTrack(tile, TrackdirToTrack(cur_td))))
				break;
			/* Safe position is all good, path valid and okay. */
			return PBSTileInfo(tile, cur_td, true);
		}

		if (!TryReserveRailTrack(tile, TrackdirToTrack(cur_td)))
			break;
	}
...
static bool TryReserveSafeTrack(const Train *v, TileIndex tile, Trackdir td, bool override_railtype)
{
	...
	{
	case VPF_NPF:
		return NPFTrainFindNearestSafeTile(v, tile, td, override_railtype);
	case VPF_YAPF:
...
public:
	VehicleOrderSaver(Train *_v) : v(_v),
	...
	bool SwitchToNextOrder(bool skip_first)
	{
		...
		{
			/* Wrap around. */
			if (this->index >= this->v->GetNumOrders())
				this->index = 0;
			...
			case OT_GOTO_DEPOT:
				...
				if ((order->GetDepotOrderType() & ODTFB_SERVICE) && !this->v->NeedsServicing())
					break;
				FALLTHROUGH;
...
static Track ChooseTrainTrack(Train *v, TileIndex tile, DiagDirection enterdir, TrackBits tracks, bool force_res, bool *got_reservation, bool mark_stuck)
{
	...
	if (do_track_reservation)
	{
		...
		if (res_dest.tile == INVALID_TILE)
		{
			...
			if (mark_stuck)
				MarkTrainAsStuck(v);
			if (changed_signal)
				SetSignalStateByTrackdir(tile, TrackEnterdirToTrackdir(best_track, enterdir), SIGNAL_STATE_RED);
	...
	else if (v->current_order.IsType(OT_LOADING) || (!v->current_order.IsType(OT_GOTO_DEPOT) && (v->current_order.IsType(OT_GOTO_STATION) ? IsRailStationTile(v->tile) && v->current_order.GetDestination() == GetStationIndex(v->tile) : v->tile == v->dest_tile)))
	{
...
bool TryPathReserve(Train *v, bool mark_as_stuck, bool first_tile_okay)
{
	...
	if (v->track == TRACK_BIT_DEPOT)
	{
		...
		else
		{
			...
			if (HasReservedTracks(next_tile, DiagdirReachesTracks(GetRailDepotDirection(v->tile))))
				return false;
		}
	}
	...
	if (Rail90DegTurnDisallowed(GetTileRailType(origin.tile), GetTileRailType(new_tile)))
		reachable &= ~TrackCrossesTracks(TrackdirToTrack(origin.trackdir));
...
{
	if (station == this->last_station_visited)
...
{
	if (old_z == v->z_pos || _settings_game.vehicle.train_acceleration_model != AM_ORIGINAL)
		return;
	...
	else
	{
		...
		if (spd <= v->gcache.cached_max_track_speed)
			v->cur_speed = spd;
	}
}
...
static bool TrainMovedChangeSignals(TileIndex tile, DiagDirection dir)
{
		...
		GetRailTileType(tile) == RAIL_TILE_SIGNALS)
	{
		...
		if (UpdateSignalsOnSegment(tile, TrackdirToExitdir(trackdir), GetTileOwner(tile)) == SIGSEG_PBS && HasSignalOnTrackdir(tile, trackdir))
		{
			...
			if (!IsPbsSignal(GetSignalType(tile, TrackdirToTrack(trackdir))))
				return true;
		}
	}
	return false;
...
uint Train::Crash(bool flooded)
{
	...
	if (this->IsFrontEngine())
	{
		...
		if (!HasBit(this->flags, VRF_TRAIN_STUCK))
...
static Vehicle *FindTrainCollideEnum(Vehicle *v, void *data)
{
	...
	if (v->type != VEH_TRAIN || Train::From(v)->track == TRACK_BIT_DEPOT)
		return nullptr;

	/* do not crash into trains of another company. */
	if (v->owner != tcc->v->owner)
		return nullptr;
...
static bool CheckTrainCollision(Train *v)
{
	...
	if (_settings_client.sound.disaster)
...
static Vehicle *CheckTrainAtSignal(Vehicle *v, void *data)
{
	...
	if (!t->IsFrontEngine() || !(t->track & TRACK_BIT_MASK))
		return nullptr;

	if (t->cur_speed > 5 || VehicleExitDir(t->direction, t->track) != exitdir)
		return nullptr;
		...
		if (v->track != TRACK_BIT_WORMHOLE)
		{
			...
			if (gp.old_tile == gp.new_tile)
			{
				...
				else
				{
					...
					if (v->IsFrontEngine() && !TrainCheckIfLineEnds(v, reverse))
						return false;
			...
			else
			{
				...
				if (prev == nullptr)
				{
					...
					if ((red_signals & chosen_track) && v->force_proceed == TFP_NONE)
					{
						...
						if (!HasSignalOnTrackdir(gp.new_tile, ReverseTrackdir(i)))
						{
							...
							if (!_settings_game.pf.reverse_at_signals || ++v->wait_counter < _settings_game.pf.wait_oneway_signal * DAY_TICKS * 2)
								return false;
						}
						else if (HasSignalOnTrackdir(gp.new_tile, i))
						{
							...
							if (!_settings_game.pf.reverse_at_signals || ++v->wait_counter < _settings_game.pf.wait_twoway_signal * DAY_TICKS * 2)
							{
								...
								if (!HasVehicleOnPos(o_tile, &exitdir, &CheckTrainAtSignal))
									return false;
							}
						}
				...
				if (v->IsFrontEngine())
				{
					...
					TileIndex crossing = TrainApproachingCrossingTile(v);
					if (crossing != INVALID_TILE && HasCrossingReservation(crossing) && _settings_client.sound.ambient)
...
static void DeleteLastWagon(Train *v)
{
	...
	if (HasReservedTracks(tile, trackbits))
	{
		...
		for (Track t : SetTrackBitIterator(remaining_trackbits))
...
static bool TrainCanLeaveTile(const Train *v)
{
	...
	if (v->track == TRACK_BIT_WORMHOLE || v->track == TRACK_BIT_DEPOT)
		return false;
...
static bool TrainLocoHandler(Train *v, bool mode)
{
	...
	if (!mode && HasBit(v->flags, VRF_TRAIN_STUCK))
	{
		...
		if (!turn_around && v->wait_counter % _settings_game.pf.path_backoff_interval != 0 && v->force_proceed == TFP_NONE)
			return true;
		if (!TryPathReserve(v))
		{
			/* Still stuck. */
	...
	else
	{
		...
		for (;;)
		{
			...
			if (CheckTrainCollision(v))
				break;
			/* Determine distance to next map position */
			adv_spd = v->GetAdvanceDistance();

			/* No more moving this tick */
			if (j < adv_spd || v->cur_speed == 0)
				break;
	...
	if (v->progress == 0)
		v->progress = j; // Save unused spd for next time, if TrainController didn't set progress
...
bool Train::Tick()
{
	...
	if (this->IsFrontEngine())
	{
		...
		if (!TrainLocoHandler(this, false))
			return false;
...
static void CheckIfTrainNeedsService(Train *v)
{
	if (Company::Get(v->owner)->settings.vehicle.servint_trains == 0 || !v->NeedsAutomaticServicing())
		return;
	if (v->IsChainInDepot())
	{
		VehicleServiceInDepot(v);
	...
	{
	case VPF_NPF:
		max_penalty = _settings_game.pf.npf.maximum_go_to_depot_penalty;
		break;
	case VPF_YAPF:
		max_penalty = _settings_game.pf.yapf.maximum_go_to_depot_penalty;
		break;
	default:
		NOT_REACHED();
	}
