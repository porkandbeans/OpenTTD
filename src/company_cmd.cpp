...
TextColour GetDrawStringCompanyColour(CompanyID company)
{
	if (!Company::IsValidID(company))
		return (TextColour)_colour_gradient[COLOUR_WHITE][4] | TC_IS_PALETTE_COLOUR;
...
{
	if (!AreCompanyManagerFaceBitsValid(cmf, CMFV_GEN_ETHN, GE_WM))
		return false;
	...
	if (!AreCompanyManagerFaceBitsValid(cmf, CMFV_EYE_COLOUR, ge))
		return false;
	for (CompanyManagerFaceVariable cmfv = CMFV_CHEEKS; cmfv < CMFV_END; cmfv++)
	{
		...
		if (!AreCompanyManagerFaceBitsValid(cmf, cmfv, ge))
			return false;
	}
...
void InvalidateCompanyWindows(const Company *company)
{
	...
	if (cid == _local_company)
		SetWindowWidgetDirty(WC_STATUS_BAR, 0, WID_S_RIGHT);
...
void SubtractMoneyFromCompany(const CommandCost &cost)
{
	...
	if (c != nullptr)
		SubtractMoneyFromAnyCompany(c, cost);
}
...
void SubtractMoneyFromCompanyFract(CompanyID company, const CommandCost &cst)
{
	...
	cost >>= 8;
	if (c->money_fraction > m)
		cost++;
	if (cost != 0)
		SubtractMoneyFromAnyCompany(c, CommandCost(cst.GetExpensesType(), cost));
}
...
static void GenerateCompanyName(Company *c)
{
	...
	if (c->name_1 != STR_SV_UNNAMED)
		return;
	if (c->last_build_coordinate == 0)
		return;
	...
	verify_name:;
		...
		{
			if (cc->name_1 == str && cc->name_2 == strp)
				goto bad_town_name;
		}

		GetString(buffer, str, lastof(buffer));
		if (Utf8StringLength(buffer) >= MAX_LENGTH_COMPANY_NAME_CHARS)
			goto bad_town_name;
...
static Colours GenerateCompanyColour()
{
	...
	for (const Company *c : Company::Iterate())
	{
		...
		for (uint j = 0; j < 2; j++)
		{
			...
			if (similar == INVALID_COLOUR)
				break;

			for (uint i = 1; i < COLOUR_END; i++)
			{
				if (colours[i - 1] == similar)
...
static void GeneratePresidentName(Company *c)
{
	...
	restart:;
		...
		if (Utf8StringLength(buffer) >= MAX_LENGTH_PRESIDENT_NAME_CHARS)
			continue;
...
Company *DoStartupNewCompany(bool is_ai, CompanyID company = INVALID_COMPANY)
{
	...
	if (is_ai && (!_networking || _network_server))
		AI::StartNew(c->index);
...
{
	if (_networking && Company::GetNumItems() >= _settings_client.network.max_companies)
		return false;
...
static void HandleBankruptcyTakeover(Company *c)
{
	...
	if (c->bankrupt_asked == MAX_UVALUE(CompanyMask))
		return;
...
void CompaniesYearlyLoop()
{
	...
	if (_settings_client.gui.show_finances && _local_company != COMPANY_SPECTATOR)
	{
		...
		{
			if (_settings_client.sound.new_year)
...
{
	if (_network_server)
		NetworkAdminCompanyRemove(company_id, (AdminCompanyRemoveReason)reason);
}
...
CommandCost CmdCompanyCtrl(TileIndex tile, DoCommandFlag flags, uint32 p1, uint32 p2, const std::string &text)
{
	...
	{ // Make a new AI company
		if (company_id != INVALID_COMPANY && company_id >= MAX_COMPANIES)
			return CMD_ERROR;

		/* For network games, company deletion is delayed. */
		if (!_networking && company_id != INVALID_COMPANY && Company::IsValidID(company_id))
			return CMD_ERROR;
	...
	{ // Delete a company
		...
		if (!_networking && Company::GetNumItems() == 1)
			return CMD_ERROR;
...
CommandCost CmdSetCompanyManagerFace(TileIndex tile, DoCommandFlag flags, uint32 p1, uint32 p2, const std::string &text)
{
	...
	if (!IsValidCompanyManagerFace(cmf))
		return CMD_ERROR;
...
CommandCost CmdSetCompanyColour(TileIndex tile, DoCommandFlag flags, uint32 p1, uint32 p2, const std::string &text)
{
	...
	if (scheme >= LS_END || (colour >= COLOUR_END && colour != INVALID_COLOUR))
		return CMD_ERROR;

	/* Default scheme can't be reset to invalid. */
	if (scheme == LS_DEFAULT && colour == INVALID_COLOUR)
		return CMD_ERROR;
	...
	if (flags & DC_EXEC)
	{
		if (!second)
		{
			if (scheme != LS_DEFAULT)
				SB(c->livery[scheme].in_use, 0, 1, colour != INVALID_COLOUR);
			if (colour == INVALID_COLOUR)
				colour = (Colours)c->livery[LS_DEFAULT].colour1;
		...
		{
			if (scheme != LS_DEFAULT)
				SB(c->livery[scheme].in_use, 1, 1, colour != INVALID_COLOUR);
			if (colour == INVALID_COLOUR)
				colour = (Colours)c->livery[LS_DEFAULT].colour2;
...
CommandCost CmdRenameCompany(TileIndex tile, DoCommandFlag flags, uint32 p1, uint32 p2, const std::string &text)
{
	...
	if (!reset)
	{
		if (Utf8StringLength(text) >= MAX_LENGTH_COMPANY_NAME_CHARS)
			return CMD_ERROR;
		if (!IsUniqueCompanyName(text))
...
static bool IsUniquePresidentName(const std::string &name)
{
	for (const Company *c : Company::Iterate())
	{
		if (!c->president_name.empty() && c->president_name == name)
			return false;
	}
...
CommandCost CmdRenamePresident(TileIndex tile, DoCommandFlag flags, uint32 p1, uint32 p2, const std::string &text)
{
	...
	if (!reset)
	{
		if (Utf8StringLength(text) >= MAX_LENGTH_PRESIDENT_NAME_CHARS)
			return CMD_ERROR;
		if (!IsUniquePresidentName(text))
			return_cmd_error(STR_ERROR_NAME_MUST_BE_UNIQUE);
	}
...
{
	if (!_settings_game.economy.give_money)
		return CMD_ERROR;
	...
	if (c->money - c->current_loan < amount.GetCost() || amount.GetCost() < 0)
		return_cmd_error(STR_ERROR_INSUFFICIENT_FUNDS);
	if (!Company::IsValidID(dest_company))
		return CMD_ERROR;
