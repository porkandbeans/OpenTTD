 ...
 */
TownNameParams::TownNameParams(const Town *t) : grfid(t->townnamegrfid), // by default, use supplied data
												type(t->townnametype)
{
...
bool VerifyTownName(uint32 r, const TownNameParams *par, TownNames *town_names)
{
	...
	if (Utf8StringLength(buf1) >= MAX_LENGTH_TOWN_NAME_CHARS)
		return false;

	if (town_names != nullptr)
	{
		if (town_names->find(buf1) != town_names->end())
			return false;
...
bool GenerateTownName(uint32 *townnameparts, TownNames *town_names)
{
	...
	for (int i = 1000; i != 0; i--)
	{
		...
		if (!VerifyTownName(r, &par, town_names))
			continue;
...
static void ReplaceWords(const char *org, const char *rep, char *buf)
{
	...
	if (strncmp(buf, org, 4) == 0)
		memcpy(buf, rep, 4); // Safe as the string in buf is always more than 4 characters long.
}
...
static void ReplaceEnglishWords(char *buf, bool original)
{
	...
	if (!original)
		ReplaceWords("Fart", "Boot", buf); // never happens with 'English (Original)'
...
static char *MakeEnglishAdditionalTownName(char *buf, const char *last, uint32 seed)
{
	...
	if (i >= 0)
		buf = strecpy(buf, _name_additional_english_prefix[i], last);
	...
	if (i >= 0)
		buf = strecpy(buf, _name_additional_english_3[i], last);
...
static char *MakeAustrianTownName(char *buf, const char *last, uint32 seed)
{
	...
	if (i >= 0)
		buf = strecpy(buf, _name_austrian_a1[i], last);
...
static char *MakeSwedishTownName(char *buf, const char *last, uint32 seed)
{
	...
	if (i >= 0)
		buf = strecpy(buf, _name_swedish_1[i], last);
...
static char *MakeDutchTownName(char *buf, const char *last, uint32 seed)
{
	...
	if (i >= 0)
		buf = strecpy(buf, _name_dutch_1[i], last);
...
static char *MakeCzechTownName(char *buf, const char *last, uint32 seed)
{
	...
	if (do_prefix)
		prefix = SeedModChance(5, lengthof(_name_czech_adj) * 12, seed) / 12;
	if (do_suffix)
		suffix = SeedModChance(7, lengthof(_name_czech_suffix), seed);
...
static char *MakeHungarianTownName(char *buf, const char *last, uint32 seed)
{
	...
	if (i < lengthof(_name_hungarian_1))
		buf = strecpy(buf, _name_hungarian_1[i], last);
...
static char *MakeDanishTownName(char *buf, const char *last, uint32 seed)
{
	...
	if (i >= 0)
		buf = strecpy(buf, _name_danish_1[i], last);
